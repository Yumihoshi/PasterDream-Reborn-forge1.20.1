...
public class TrueGrassSwordPr1Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		if (entity == null)
			return;
		if (itemstack.getOrCreateTag().getBoolean("skill") != true) {
			itemstack.getOrCreateTag().putBoolean("skill", true);
			itemstack.getOrCreateTag().putDouble("paster_atk", (7.5 * itemstack.getEnchantmentLevel(Enchantments.SHARPNESS) * 0.5 + ((LivingEntity) entity).getAttribute(net.minecraft.world.entity.ai.attributes.Attributes.ATTACK_DAMAGE).getValue()));
			itemstack.getOrCreateTag().putDouble("skill_multiplier", ((LivingEntity) entity).getAttribute(PasterdreamModAttributes.SKILLMULTIPLIER.get()).getValue());
					...
					for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
						...
						if (itemstackiterator.is(ItemTags.create(new ResourceLocation("pasterdream:skill")))) {
							if (entity instanceof Player _player)
								_player.getCooldowns().addCooldown(itemstackiterator.getItem(), (int) (80 * ((LivingEntity) entity).getAttribute(PasterdreamModAttributes.SKILLCD.get()).getValue()));
						}
					}
				}
			}
