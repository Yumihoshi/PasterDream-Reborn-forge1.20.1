...
import net.pasterdream.procedures.BoneWingFireBallPr0Procedure;
...
public class BoneWingFireBallProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public BoneWingFireBallProjectileEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(PasterdreamModEntities.BONE_WING_FIRE_BALL_PROJECTILE.get(), world);
	}
	...
	public void tick() {
		super.tick();
		BoneWingFireBallPr0Procedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
		if (this.inGround)
			this.discard();
	}
	...
	public static BoneWingFireBallProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		BoneWingFireBallProjectileEntity entityarrow = new BoneWingFireBallProjectileEntity(PasterdreamModEntities.BONE_WING_FIRE_BALL_PROJECTILE.get(), entity, world);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("pasterdream:bone_wing_fire_ball")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static BoneWingFireBallProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		BoneWingFireBallProjectileEntity entityarrow = new BoneWingFireBallProjectileEntity(PasterdreamModEntities.BONE_WING_FIRE_BALL_PROJECTILE.get(), entity, entity.level());
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("pasterdream:bone_wing_fire_ball")), SoundSource.PLAYERS, 1,
