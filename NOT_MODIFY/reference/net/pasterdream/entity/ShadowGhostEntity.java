...
public class ShadowGhostEntity extends Monster implements GeoEntity {
	...
	public ShadowGhostEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(PasterdreamModEntities.SHADOW_GHOST.get(), world);
	}

	public ShadowGhostEntity(EntityType<ShadowGhostEntity> type, Level world) {
		super(type, world);
		xpReward = 2;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new Goal() {
			...
			public boolean canContinueToUse() {
				return ShadowGhostEntity.this.getMoveControl().hasWanted() && ShadowGhostEntity.this.getTarget() != null && ShadowGhostEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = ShadowGhostEntity.this.getTarget();
			...
			public void tick() {
				LivingEntity livingentity = ShadowGhostEntity.this.getTarget();
				if (ShadowGhostEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					ShadowGhostEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = ShadowGhostEntity.this.distanceToSqr(livingentity);
					if (d0 < 5) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						ShadowGhostEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 0.6);
					}
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				...
				double dir_x = ShadowGhostEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = ShadowGhostEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = ShadowGhostEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public static void init() {
		SpawnPlacements.register(PasterdreamModEntities.SHADOW_GHOST.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(ShadowGhostEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
