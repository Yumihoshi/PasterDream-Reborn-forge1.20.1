...
import net.pasterdream.procedures.ThundercloudPr1Procedure;
import net.pasterdream.procedures.ThundercloudPr0Procedure;
...
public class ThundercloudEntity extends Monster implements GeoEntity {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(ThundercloudEntity.class, EntityDataSerializers.BOOLEAN);
	...
	public ThundercloudEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(PasterdreamModEntities.THUNDERCLOUD.get(), world);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new RandomStrollGoal(this, 0.4, 20) {
			...
			protected Vec3 getPosition() {
				...
				double dir_x = ThundercloudEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = ThundercloudEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = ThundercloudEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public SoundEvent getDeathSound() {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.candle.extinguish"));
	}
	...
	public boolean hurt(DamageSource source, float amount) {
		ThundercloudPr1Procedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	...
	public void baseTick() {
		super.baseTick();
		ThundercloudPr0Procedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
