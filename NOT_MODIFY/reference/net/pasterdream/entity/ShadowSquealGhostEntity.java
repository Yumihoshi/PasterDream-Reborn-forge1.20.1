...
public class ShadowSquealGhostEntity extends Monster implements RangedAttackMob, GeoEntity {
	...
	public ShadowSquealGhostEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(PasterdreamModEntities.SHADOW_SQUEAL_GHOST.get(), world);
	}

	public ShadowSquealGhostEntity(EntityType<ShadowSquealGhostEntity> type, Level world) {
		super(type, world);
		xpReward = 2;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new Goal() {
			...
			public boolean canContinueToUse() {
				return ShadowSquealGhostEntity.this.getMoveControl().hasWanted() && ShadowSquealGhostEntity.this.getTarget() != null && ShadowSquealGhostEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = ShadowSquealGhostEntity.this.getTarget();
			...
			public void tick() {
				LivingEntity livingentity = ShadowSquealGhostEntity.this.getTarget();
				if (ShadowSquealGhostEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					ShadowSquealGhostEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = ShadowSquealGhostEntity.this.distanceToSqr(livingentity);
					if (d0 < 5) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						ShadowSquealGhostEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 0.6);
					}
		...
		this.goalSelector.addGoal(3, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				...
				double dir_x = ShadowSquealGhostEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = ShadowSquealGhostEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = ShadowSquealGhostEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public static void init() {
		SpawnPlacements.register(PasterdreamModEntities.SHADOW_SQUEAL_GHOST.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(ShadowSquealGhostEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
