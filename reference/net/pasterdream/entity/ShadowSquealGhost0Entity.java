...
import net.pasterdream.procedures.ShadowSquealGhost0Pr0Procedure;
...
public class ShadowSquealGhost0Entity extends Monster implements RangedAttackMob, GeoEntity {
	...
	public ShadowSquealGhost0Entity(PlayMessages.SpawnEntity packet, Level world) {
		this(PasterdreamModEntities.SHADOW_SQUEAL_GHOST_0.get(), world);
	}

	public ShadowSquealGhost0Entity(EntityType<ShadowSquealGhost0Entity> type, Level world) {
		super(type, world);
		xpReward = 2;
		setNoAi(false);
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "shadow_squeal_wave_0");
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new Goal() {
			...
			public boolean canContinueToUse() {
				return ShadowSquealGhost0Entity.this.getMoveControl().hasWanted() && ShadowSquealGhost0Entity.this.getTarget() != null && ShadowSquealGhost0Entity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = ShadowSquealGhost0Entity.this.getTarget();
			...
			public void tick() {
				LivingEntity livingentity = ShadowSquealGhost0Entity.this.getTarget();
				if (ShadowSquealGhost0Entity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					ShadowSquealGhost0Entity.this.doHurtTarget(livingentity);
				} else {
					double d0 = ShadowSquealGhost0Entity.this.distanceToSqr(livingentity);
					if (d0 < 5) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						ShadowSquealGhost0Entity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 0.6);
					}
		...
		this.goalSelector.addGoal(3, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				...
				double dir_x = ShadowSquealGhost0Entity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = ShadowSquealGhost0Entity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = ShadowSquealGhost0Entity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public boolean hurt(DamageSource source, float amount) {
		ShadowSquealGhost0Pr0Procedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	...
	public static void init() {
		SpawnPlacements.register(PasterdreamModEntities.SHADOW_SQUEAL_GHOST_0.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(ShadowSquealGhost0Entity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
