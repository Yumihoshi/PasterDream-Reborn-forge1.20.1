...
public class DyedreamSwordPr1Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getOrCreateTag().getBoolean("skill") == false) {
			...
			if ((entity instanceof LivingEntity _livEnt7 && _livEnt7.hasEffect(PasterdreamModMobEffects.DYEDREAM_ARMOR_BUFF.get())) == true) {
				{
						...
						for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
							...
							if (itemstackiterator.is(ItemTags.create(new ResourceLocation("pasterdream:skill")))) {
								if (entity instanceof Player _player)
									_player.getCooldowns().addCooldown(itemstackiterator.getItem(), (int) (60 * ((LivingEntity) entity).getAttribute(PasterdreamModAttributes.SKILLCD.get()).getValue()));
							}
						}
					}
				}
			} else {
				{
					...
					if (_iitemhandlerref.get() != null) {
						for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
							...
							if (itemstackiterator.is(ItemTags.create(new ResourceLocation("pasterdream:skill")))) {
								if (entity instanceof Player _player)
									_player.getCooldowns().addCooldown(itemstackiterator.getItem(), (int) (100 * ((LivingEntity) entity).getAttribute(PasterdreamModAttributes.SKILLCD.get()).getValue()));
							}
						}
					}
				}
